...
状態: 2026-09-26の設計契約。既存の[人格v2](AGENT_INTERFACES_v2.md)の外側を明確にする。[共通の人格入出力型](../packages/ai/personality.ts)をA1の[小fixture](../packages/sim/autonomy-a1.ts)とA2の買物係に適用した。ただし20人全員の共通Gateway、刺激配送、ActionProcess、全要件の受入テストはまだ存在しない。最初は20人の経済に使うが、職業、文化、動物、資源や判断実装の種類をここへ列挙しない。
...
ここでいう「世界の物理」は、質量・容量・位置・時間・動力・消耗・生産/消費・情報の到達といった局所的な因果法則を指す。所有権や契約などの制度も、行為の可否や権利/債務を判定する世界の制約として扱う。simは仕事の成功や共同体の安定を目標として持たず、職業別の日課や不足時の救済を全員へ指令しない。安定が生まれるなら、各人の身体的必要、役割、習慣、期待、利害から選んだ行為が、資源と制度の制約下で結びついた結果である。失敗した場合も中央で補正せず、因果と制約を観測して本人の行動則や制度を調整する。
