...
状態: 2026-09-26の設計契約。既存の[人格v2](AGENT_INTERFACES_v2.md)の外側を明確にする。A1の[小fixture](../packages/sim/autonomy-a1.ts)では一部の境界を実装・検証したが、共通コードAPI・20人世界のセーブ形式・全要件の受入テストはまだ存在しない。最初は20人の経済に使うが、職業、文化、動物、資源や判断実装の種類をここへ列挙しない。
