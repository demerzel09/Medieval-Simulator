...
状態: 2026-09-26の実装前設計。コード、コンテンツ、保存形式、受入試験は未変更。既存の[人格境界v2](AGENT_INTERFACES_v2.md)、[ルーティン](DATA_DRIVEN_ROUTINES.md)、[物体・包含モデル](PHYSICAL_OBJECT_MODEL.md)を接続する。
...
共通能力層も市場も未実装で、人数規模/複数便の性能は未計測。各段階で「誰が知り、判断し、働き、どの物が動き、誰が報酬を受けたか」を人物ID・物体ID・Task ID・Event原因で追う。
