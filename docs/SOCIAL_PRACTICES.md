...
`applicable` は本人が知る場面と定義から**使用可能な慣行を探す**処理であり、人物の最終選択ではない。人格の判断が慣行の起動を提案した後、`instantiate` に相当する共通ランナーがTaskへ展開する。受け手は自分の文化と状況でTaskを認識・判断するため、依頼人と同じ慣行を知っている必要はない。[v2の全体図](AGENT_INTERFACES_v2.md#何をインターフェースにするか)を参照。
