...
実装前に確定すべき契約と既存コードとの差は[実装前監査](ECONOMY_IMPLEMENTATION_READINESS.md)に整理した。E0の計測は着手可能だが、E1では在庫の正本・取引状態・人物の時間占有などを先に固定する。
...
| 試験 | 合格条件 |
| --- | --- |
| 基準90日 | 全PersonのIDと日ごとの勤務/配達/買物/食事の結果を原因Eventで追える。30日以降も実生産・配送・売買・食事が継続し、世帯の未払・在庫・資金が説明できる。恣意的な全世帯飢餓ゼロは要求しない |
...
| 再現と負荷 | seed、Command、保存状態、コンテンツ版から同じEventと台帳へ再現。1,000人90日で必要な性能を**実測**し、現行値を新設計の達成値に流用しない |
...
2. **E1: 食料の実物流通。** 所在地付き在庫、予約、注文、運搬人/売り手/買物係のTask、受領時清算を小集落fixtureで通す。この段階の賃金・家計は既存方式を暫定利用し、経済の自律性を達成したとは判定しない。人格v2全体の移行を待たず、既存ルールAIを共通境界の最小実装として使う。
...
各段で受入fixture、保存/再生、不変条件、既存シナリオ回帰を実行する。新経済の停止や不足を隠すため旧共同口座から無断補填しない。人間試遊は引き続きM3の別の未完了条件である。E0〜E3は**今後の作業単位**で、いずれもこの文書作成時点では未実装・未検証。
