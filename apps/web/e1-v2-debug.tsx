...
import { advanceLocalV2, localV2Summary, newLocalWorldA2, newLocalWorldA3Income, newLocalWorldV2, submitLocalV2, type LocalWorldV2 } from "../../packages/sim/local-economy-v2";
...
function makeWorld(scenario: Scenario, minute: number, autonomousBuyers: boolean, incomeExperiment: boolean) {
  const w = incomeExperiment ? newLocalWorldA3Income() : autonomousBuyers ? newLocalWorldA2() : newLocalWorldV2();
...
export default function E1V2Debug({ autonomousBuyers = false, incomeExperiment = false }: { autonomousBuyers?: boolean; incomeExperiment?: boolean }) {
  const [scenario, setScenario] = useState<Scenario>("baseline");
  const [minute, setMinute] = useState(0);
  const [playing, setPlaying] = useState(false);
  const [focus, setFocus] = useState("F0");
  const w = useMemo(() => makeWorld(scenario, minute, autonomousBuyers, incomeExperiment), [scenario, minute, autonomousBuyers, incomeExperiment]);
  useEffect(() => {
    if (!playing) return;
  ...
  return <div className="e1-debug">
    <header className="e1-top"><div><p className="eyebrow">{incomeExperiment ? "A3所得試作 · 3日" : autonomousBuyers ? "LOCAL FOOD A2 · ECONOMIC ACTOR DECISIONS" : "LOCAL FOOD V2 · PHYSICAL TRUTH"}</p><h1>20人の集落 · {incomeExperiment ? "実働・賃金・欠食の検証" : autonomousBuyers ? "買物・販売・農業・運搬の自律デバッグ" : "物体木デバッグ"}</h1></div><div><a href="/?e1=debug">旧E1</a> · <a href="/?e1=v2">物体木版</a> · <a href="/?e1=a2">自律A2</a> · <a href="/?e1=a3-income">A3所得試作</a> · <a href="/">90日ゲーム</a></div></header>
    ...
    <main className="e1-layout"><section className="e1-map-panel"><p>場所・人物・荷車・保管具をクリックすると、物理親と所有者、再帰負荷、容量、予約、行動を確認できます。</p>
      ...
      {w.people[focus] && <><h2>{focus}の仕事と行動</h2><p>世帯: {w.people[focus].householdId} · 体力: {w.people[focus].energy} · 空腹: {w.people[focus].hunger}</p>
        ...
        {focus === "S" && w.sellerActor && <p>次の判断: {clock(w.sellerActor.nextWakeAt)} · 市場で見た注文: {w.sellerActor.knownOrderIds.join("、") || "なし"} · 届いた収穫証拠: {w.sellerActor.knownWorkProofIds?.length ?? 0}件 · 補充依頼: {w.sellerActor.requestEventId ?? "なし"}</p>}
        ...
        {focus === "C" && w.carrierActor && <p>次の判断: {clock(w.carrierActor.nextWakeAt)} · 現地で受けた配送: {w.carrierActor.shipmentId ?? "なし"} · 積荷の収穫証拠: {w.shipments.find((shipment) => shipment.id === w.carrierActor?.shipmentId)?.workProofIds?.length ?? 0}件 · 運搬Task: {w.carrierActor.taskId ?? "なし"}</p>}
