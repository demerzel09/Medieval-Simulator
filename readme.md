...
VS Codeでは「実行とデバッグ」（F5）から、ブラウザ版、90日シナリオ、E0/E1の各ケース、検証・ベンチマークを選べます。[launch.json](.vscode/launch.json)はこの作業環境の `~/.local/node` を参照します。Nodeを別の場所に入れた環境では、そのパスを変更してください。

E1の空間デバッグは `http://127.0.0.1:5173/?e1=debug`、またはF5の「経済 E1: 空間デバッグ画面」で開きます。町・4軒の家・農場・市場、20人の移動と食料の所在地を時刻スライダーで再生できます。人物を選ぶと世帯、仕事、因果Eventを見られます。[画面例](artifacts/e1-spatial-debug.png)。この画面は独立したE1実験であり、90日ゲームの世界とはまだ接続していません。
...
2026-09-26の検証結果：単体・シナリオ **37/37成功**、ブラウザ **5/5成功**。型検査・本番ビルド・コンテンツ検査も成功しました。ブラウザ検証には保存・再開、90日進行、JSON読込、会戦表示、幅390pxの画面とE1空間デバッグを含みます。既存ゲームの[検証記録](artifacts/validation.json)と[E1画面例](artifacts/e1-spatial-debug.png)を保存しています。
...
次の設計・実装の優先は[市民経済の閉路](docs/ECONOMY_LOOP_DESIGN.md)です。E0の現行経済計測に続き、E1の独立20人集落で、実在の農民・運搬人・売り手・買物係による食料流通を実装しました。現行90日ゲームの国別共同市場はまだこの新経済へ移行していません。
