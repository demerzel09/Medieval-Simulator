...
これらを扱う[自律行動の外側のインターフェース](docs/AUTONOMY_INTERFACE_CONTRACT.md)を設計しました。本人へ届く刺激、本人の判断と待機、simによる試行と利用権、進行中行動と結果を共通の境界とし、人格v2の意味分割を維持します。現在は文書設計のみで、A1以降の実装と検証はまだです。
