...
次の開発は[自律的な箱庭の開発順](docs/AUTONOMOUS_SANDBOX_ROADMAP.md)に沿います。20人の仕事・食料・所得・荷車の保守が平常時に90日続くことを確かめてから、故障と不作を試します。軍事の新機能、空腹感の精密化、人間関係、木々や動物の生態系はこの検証より後です。この90日安定実験と偶発トラブルはまだ未実装です。
...
**A1小fixture:** `npm run autonomy:a1`（VS Codeでは「自律 A1: 二人と一つの道具」）で、二人が同じ道具を必要とする仕事の依頼を受ける6分間を因果Event付きで表示します。一人目が作業中、二人目は待機し、道具の解放通知で自分から着手します。A1のコードは[autonomy-a1.ts](packages/sim/autonomy-a1.ts)。A1自体は独立した小世界で、90日安定の検証ではありません。

**A2買物係:** `npm run autonomy:a2`で20人経済の3日間の買物係の判断と売買を表示します。ブラウザの`/?e1=a2`、またはVS Code「自律 A2: 買物係の空間デバッグ画面」では、人物をクリックして判断Event、移動、食料・現金を時刻ごとに確認できます。A1と買物係は[共通の人格入出力型](packages/ai/personality.ts)を使います。旧物体木版`/?e1=v2`は比較用に残しています。売り手・農民・運搬人などの本人主導化と90日安定は今後の作業です。

## 次の作業

1. A1で本人の行動状態、局所的判断、時間積算・中断・保存再開の小fixtureを実装する。
2. A2でE1の買物・販売・農作業・輸送を本人主導へ移し、A3で所得と荷車保守の閉路を平常90日で検証する。
