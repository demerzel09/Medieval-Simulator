...
個人所有の新しい二つの独立実験も起動できます。`npm run autonomy:life -- 30` は世帯・農場・市場のない3人の採集生活、`npm run autonomy:market -- 7` はS所有の市場でFの収穫をCが運び、Sが自分の資金で仕入れて売る取引です。ブラウザの `/?individual=life` と `/?individual=market`、またはF5の「自律: 制度なし採集生活の空間デバッグ画面」「自律: S個人市場の空間デバッグ画面」で1〜7日の日末状態と全員の行動・所有権・因果Eventを確認できます。CLIの引数を90にすると90日計測になります。[仕組みと測定結果](docs/INDIVIDUAL_ECONOMY_IMPLEMENTATION.md)に制約を記録しました。個人市場は買主の所得源がないため90日安定に失敗しており、本編やA3へは未接続です。
...
2026-09-27の検証結果：単体・シナリオ **100/100成功**、ブラウザ **11/11成功**。型検査・コンテンツ検査・本番ビルドも成功しました。行動一覧を全件表示に変更した後、新画面2/2を再確認しました。ブラウザ検証には保存・再開、90日進行、JSON読込、会戦表示、幅390pxの画面とA3を含む空間デバッグを含みます。既存ゲームの[検証記録](artifacts/validation.json)と旧[E1画面例](artifacts/e1-spatial-debug.png)、A3の[90日計測](artifacts/a3-income-90d-summary.json)を保存しています。A3の計測は固定配置・単一seedに限ります。

本編で未実装の主な範囲は、個人市場との統合、融資の元本実行、捕虜交換、自由布陣・医療、政治運動です。LLM実接続・出生継承・3D化・公開デプロイは今回の対象外です。
...
1. 個人採集生活とS個人市場を同一世界に統合し、買主の食事と所得、農民・運搬人・Sの身体状態を実際の行動へつなぐ。取引停止を救済せず、仕事による収入から再び購入できるか検証する。
