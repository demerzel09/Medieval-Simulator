...
2026-09-27の検証結果：単体・シナリオ **91/91成功**、ブラウザ **9/9成功**。型検査・本番ビルド・コンテンツ検査も成功しました。ブラウザ検証には保存・再開、90日進行、JSON読込、会戦表示、幅390pxの画面とA3を含む空間デバッグを含みます。既存ゲームの[検証記録](artifacts/validation.json)と旧[E1画面例](artifacts/e1-spatial-debug.png)、A3の[90日計測](artifacts/a3-income-90d-summary.json)を保存しています。A3の計測は固定配置・単一seedに限ります。
