...
これらを扱う[自律行動の外側のインターフェース](docs/AUTONOMY_INTERFACE_CONTRACT.md)を設計しました。本人へ届く刺激、本人の判断と待機、simによる試行と利用権、進行中行動と結果を共通の境界とし、人格v2の意味分割を維持します。共通人格v2と20人経済への接続はまだ設計段階です。

**A1小fixture:** `npm run autonomy:a1`（VS Codeでは「自律 A1: 二人と一つの道具」）で、二人が同じ道具を必要とする仕事の依頼を受ける6分間を因果Event付きで表示します。一人目が作業中、二人目は待機し、道具の解放通知で自分から着手します。A1のコードは[autonomy-a1.ts](packages/sim/autonomy-a1.ts)。20人経済への接続や90日安定はまだ実装していません。
