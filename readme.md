...
**A2買物係・売り手・農民:** `npm run autonomy:a2`で20人経済の3日間の注文、補充、出勤、収穫、売買と各人の判断を表示します。`npm run dev`で起動して`http://127.0.0.1:5173/?e1=a2`を開くか、VS Code「自律 A2: 買物係・売り手・農民の空間デバッグ画面」を使ってください。B0〜B3・S・F0〜F7をクリックすると、次の判断、勤務や注文の状態、因果Event、移動、食料・現金を確認できます。[共通の人格入出力型](packages/ai/personality.ts)に各職務のデータを渡します。旧物体木版`/?e1=v2`は比較用です。運搬人の本人主導化と20人の90日安定は今後の作業です。

## 次の作業

1. A2で運搬人の配車・積載・配送を本人主導へ移し、食事/休息と家での荷下ろしも順次接続する。
2. A1で検証した分刻みの行動進捗・中断をE1に移し、A3で所得と荷車保守の閉路を平常90日で検証する。
