...
現行の費用は出発・作業時に一括処理します。雨・渋滞・襲撃などを道中で扱い、仕事の進捗に応じて体力を消費するための[時間と負荷で進む行動の設計](docs/ACTION_PROCESS_DESIGN.md)を追加しました。共通実行器は未実装です。

## 次の作業

1. E1の時間積算・中断・保存再開を小fixtureで検証し、徒歩/荷車の共通実行器へ移す。その後E2で4世帯と事業の賃金・仕入・実働による資金循環を設計・実装する。共通ルーティン展開と本人へ届いた情報からの判断を接続し、既存90日ゲームへの移行方法を決める。
